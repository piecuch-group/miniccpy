...
def rdm2_ccsdt(L, T):
    ...
    rdm2_oooo = (
                ...
                +(1.0/6.0)*np.einsum('efgijm,efgklm->ijkl', l3, t3, optimize=True)

                +0.5*np.einsum('efgijm,ek,fglm->ijkl', l3, t1, t2, optimize=True)
                +0.5*np.einsum('efgijm,el,fgkm->ijkl', l3, t1, t2, optimize=True)
    ) 

    #ooov Gamma_ijka --ija--.--k--
    rdm2_ooov = (-np.einsum('eaij,ek->ijka', l2, t1, optimize=True)
                 
                 -np.einsum('eafijm,efkm->ijka', l3, t2, optimize=True)
    )             
    rdm2_oovo = -np.transpose(rdm2_ooov, (0,1,3,2))
