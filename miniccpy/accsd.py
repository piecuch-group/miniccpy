...
def doubles_residual(t1, t2, f, g, g_unsym, o, v, acc_scaling):
    """Compute the projection of the CCSD Hamiltonian on doubles
        X[a, b, i, j] = < ijab | (H_N exp(T1+T2))_C | 0 >
    """

    d1, d2, d3, d4, d5 = acc_scaling
    H1, H2 = get_ccs_intermediates(t1, f, g, o, v)

    # intermediates
    I_oo = H1[o, o] + 0.5 * d4 * np.einsum("mnef,efin->mi", g[o, o, v, v], t2, optimize=True)

    I_vv = H1[v, v] - 0.5 * d3 * np.einsum("mnef,afmn->ae", g[o, o, v, v], t2, optimize=True)

    I_voov = H2[v, o, o, v] + 0.5 * (
            (d1 - d2) * np.einsum("mnef,afin->amie", g_unsym[o, o, v, v], t2, optimize=True)
            + d2 * np.einsum("mnef,afin->amie", g[o, o, v, v], t2, optimize=True)
    )

    I_oooo = H2[o, o, o, o] + 0.5 * d5 * np.einsum("mnef,efij->mnij", g[o, o, v, v], t2, optimize=True)
...
def kernel(fock, g_unsym, o, v, maxit, convergence, diis_size, n_start_diis, out_of_core, energy_shift, acc_scaling):
    ...
    # Create antisymmetric two-body integrals
    g = g_unsym - np.transpose(g_unsym, (0, 1, 3, 2))
    ...
    for idx in range(maxit):
        ...
        residual_doubles = doubles_residual(t1, t2, fock, g, g_unsym, o, v, acc_scaling)
