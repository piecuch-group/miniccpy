...
def calc_LR(L, R, nocc, nunocc):
    # unpack L
    l1 = L[:nocc*nocc].reshape(nocc, nocc)
    l2 = L[nocc*nocc:].reshape(nocc, nocc, nunocc, nocc)
    # unpack R
    r1, r2 = R
    # compute LR
    LR = 0.5 * np.einsum("ij,ij->", l1, r1, optimize=True)
    LR += (1.0 / 6.0) * np.einsum("ijck,ijck->", l2, r2, optimize=True)
    return LR
