...
def rdm1_ccsdt_other(L, T):
    ...
    rdm1_oo = (
                    -np.einsum('ei,ej->ij', l1, t1, optimize=True)
                    -0.5*np.einsum('efim,efjm->ij', l2, t2, optimize=True)
                    -(1.0/12.0)*np.einsum('efgimn,efgjmn->ij', l3, t3, optimize=True)
              )
    ...
    rdm1_vo = (
                    ...
                    -0.5*np.einsum('efmn,afmn,ei->ai', l2, t2, t1, optimize=True)
                    +0.25*np.einsum('efmn,aefimn->ai', l2, t3, optimize=True)
                    -0.25*np.einsum('efgmno,efmi,agno->ai', l3, t2, t2, optimize=True)
                    -(1.0/12.0)*np.einsum('efgmno,ei,afgmno->ai', l3, t1, t3, optimize=True)
                    -(1.0/12.0)*np.einsum('efgmno,am,efgino->ai', l3, t1, t3, optimize=True)
                )
    rdm1_vo += t1.copy()
    ...
    rdm1_vv = (
                    +np.einsum('bm,am->ab', l1, t1, optimize=True)
                    +0.5*np.einsum('bemn,aemn->ab', l2, t2, optimize=True)
                    +(1.0/12.0)*np.einsum('befmno,aefmno->ai', l3, t3, optimize=True)
                )
